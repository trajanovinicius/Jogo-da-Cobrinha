//que vai ser em javascript
window.onload = function () {
  //página vai carregar e tudo vai começar
  ...
  setInterval(game, 110); //chamando uma função no nosso jogo em milisegundos, velocidade, quando maior mais lento a cobra
  ...
  tail = 4; //tamanho da calda da cobra

  function game() {
    //cada vez que a cobra andar um pouco eu tenho que pintar toda a tela de novo
    ...
    if (px < 0) {
      //se ela chega na borda, ela tem que ir para o final do palco
    ...
    ctx.fillStyle = "white"; //pintar o lugar onde fica a maça
    ctx.fillRect(ax * tp, ay * tp, tp, tp); //posições que eu quero pintar, tp tamanho da peça

    ctx.fillStyle = "red"; //pintar a cobra e rastro dela, vai mudar um pouco pelo rastro dela!
    for (var i = 0; i < trail.length; i++) {
      //px e py é sempre o tamanho da cabeça da cobra
      ...
      if (trail[i].x == px && trail[i].y == py) {
        //se px e py baterem em alguma parte do rastro, então game over
    ...
    while (trail.length > tail) {
      //rastro maior que o tamanho da calda, então
      trail.shift(); //tira o primeiro elemento do array
    }

    if (ax == px && ay == py) {
      //se a posição da maçã for igual a posição da cobra
  ...
  function keyPush(event) {
    //definir a nossa movimentação, como controlar a cobra

    switch (
      event.keyCode //qual tecla foi pressionada, código do meu evento
    ) {
      case 37: // Left
